...
class UploadMediaViewController: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    func showAlert()
    {
        if(self.mediaID==nil)
        {
            let alertController = UIAlertController(title: "OPGSDKv1.0", message: "Media upload failed", preferredStyle: .alert)
        ...
        {
            let alertController = UIAlertController(title: "OPGSDKv1.0", message: "Uploaded media ID is \(self.mediaID!)", preferredStyle: .alert)
